...
## is unchanged (type="any", ignore.strand=T).
...
## All outputs are written to output_any/ so that the matmiRNA-within strategy
## (output_matmiRNA/) and matmiRNA-union strategy (output_union/) are kept
## intact for the three-way overlap-rule comparison.
...
### any overlap (>= 1 bp): return indices of reads overlapping a feature
any.overlap.ids = function(reads.gr, feature.gr){
  ol = findOverlaps(reads.gr, feature.gr, type = "any")
  return(unique(queryHits(ol)))
}
...
for(i in seq(along = files)){
      ...
      for(j in 1:length(genomicFeature.id)){
	    ...
	    hit.ids = any.overlap.ids(test.bam.gr, get(feature.name))
	    test.bam.hit.gr = test.bam.gr[hit.ids]
	    test.bam.hit.gr$region = rep("NA", length(test.bam.hit.gr))
	    if(feature.id == "matmiRNA"){
	    		  test.bam.hit.gr = mygeneFeature(bam = "test.bam.hit.gr", type = "any")
			  matmiRNA.geneFeature.count.df = count.by(test.bam.hit.gr$region, test.bam.hit.gr$count)
			  write.table(matmiRNA.geneFeature.count.df, file = paste("../../../output_any/tables/", sample, ".matmiRNA.annotation.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
			  matmiRNA.size.count.df = count.by(width(test.bam.hit.gr), test.bam.hit.gr$count)
			  write.table(matmiRNA.size.count.df, file = paste("../../../output_any/tables/", sample, ".matmiRNA.size.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
	    ...
	    if(feature.id == "snoRNA"){
	    		  test.bam.hit.gr = mygeneFeature("test.bam.hit.gr", type = "any")
			  snoRNA.geneFeature.count.df = count.by(test.bam.hit.gr$region, test.bam.hit.gr$count)
			  write.table(snoRNA.geneFeature.count.df, file = paste("../../../output_any/tables/", sample, ".snoRNA.annotation.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
			  snoRNA.size.count.df = count.by(width(test.bam.hit.gr), test.bam.hit.gr$count)
			  write.table(snoRNA.size.count.df, file = paste("../../../output_any/tables/", sample, ".snoRNA.size.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
	    ...
	    if(feature.id == "piRNA"){
			  ...
			  write.table(piRNA.geneFeature.count.df, file = paste("../../../output_any/tables/", sample, ".piRNA.annotation.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
			  piRNA.size.count.df = count.by(width(test.bam.hit.gr), test.bam.hit.gr$count)
			  write.table(piRNA.size.count.df, file = paste("../../../output_any/tables/", sample, ".piRNA.size.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
	    ...
	    if(feature.id == "tRNA"){
			  ...
			  write.table(tRNA.geneFeature.count.df, file = paste("../../../output_any/tables/", sample, ".tRNA.annotation.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
			  tRNA.size.count.df = count.by(width(test.bam.hit.gr), test.bam.hit.gr$count)
			  write.table(tRNA.size.count.df, file = paste("../../../output_any/tables/", sample, ".tRNA.size.count.txt", sep = ""),
			   sep = "\t", quote = F, col.names = T, row.names = F)
      ...
      save(reads.bam.annotated.gr, file = paste("../../../output_any/rdata/", sample, ".bam.annotated.gr.RData", sep = ""))
      write.table(read.annotation.count.df, file = paste("../../../output_any/tables/", sample, ".read.annotation.count.txt", sep = ""), quote = F, sep = "\t", col.names = T, row.names = F)
      write.table(read.size.count.df, file = paste("../../../output_any/tables/", sample, ".read.size.count.txt", sep = ""), quote = F, sep = "\t", col.names = T, row.names = F)
}

## ----- consolidated count tables (unique reads vs all reads) -------------------
write.csv(unique.all.tab, "../../../output_any/tables/Table2a_annotation_count_unique_reads.csv", row.names = FALSE)
write.csv(reads.all.tab, "../../../output_any/tables/Table2b_annotation_count_all_reads.csv", row.names = FALSE)
print("Tables saved to ../../../output_any/tables/: Table2a_annotation_count_unique_reads.csv, Table2b_annotation_count_all_reads.csv")
