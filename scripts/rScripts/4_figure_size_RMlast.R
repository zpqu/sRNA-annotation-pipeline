...
###for all small RNAs
all.read.df = NULL
files = list.files(path = "../../output/", pattern = ".size.count.txt$")
...
#		   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
      ...
      ggsave(p.sample.class.barplot, file = paste0("../../plots/Fig2a.", class.list[j], "_size_barplot.pdf"), width = fig.width, height = fig.height)
...
#		   theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
		   facet_wrap(~sample, ncol = 4)
      ggsave(p.sample.class.percent.plot, file = paste0("../../plots/Fig2b.", class.list[j], "_size_barplot.percentage.pdf"), width = fig.width, height = fig.height)
}